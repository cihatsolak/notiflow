...
internal sealed class TenantPermissionManager : ITenantPermissionService
{
    ...
    public async Task<Result<TenantPermissionResponse>> GetPermissionsAsync(CancellationToken cancellationToken)
    {
        ...
        {
            return Result<TenantPermissionResponse>.Failure(StatusCodes.Status404NotFound, _localizer[ValidationErrorCodes.TENANT_PERMISSION_NOT_FOUND]);
        }

        return Result<TenantPermissionResponse>.Success(StatusCodes.Status200OK, _localizer[ValidationErrorCodes.GENERAL_SUCCESS], tenantPermission);
    }
    ...
    public async Task<Result<EmptyResponse>> UpdateAsync(TenantPermissionRequest request, CancellationToken cancellationToken)
    {
        ...
        if (tenantPermission is null)
        {
            return Result<EmptyResponse>.Failure(StatusCodes.Status404NotFound, _localizer[ValidationErrorCodes.TENANT_PERMISSION_NOT_FOUND]);
        }
        ...
        return Result<EmptyResponse>.Success(StatusCodes.Status204NoContent, _localizer[ValidationErrorCodes.TENANT_PERMISSION_NOT_FOUND]);
    }
}
