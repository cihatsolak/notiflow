...
public sealed class AddDeviceCommandHandler : IRequestHandler<AddDeviceCommand, Result<int>>
{
    ...
    public async Task<Result<int>> Handle(AddDeviceCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            return Result<int>.Failure(StatusCodes.Status404NotFound, _localizer[ValidationErrorCodes.DEVICE_EXISTS]);
        }
        ...
        return Result<int>.Success(StatusCodes.Status201Created, _localizer[ValidationErrorCodes.DEVICE_ASSOCIATED_CUSTOMER_ADDED], device.Id);
    }
}
