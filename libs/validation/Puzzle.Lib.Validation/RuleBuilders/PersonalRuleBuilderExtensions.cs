...
public static class PersonalRuleBuilderExtensions
{
    ...
    /// <param name="errorMessage">The error message to display if the validation fails.</param>
    /// <returns>The rule builder options.</returns>
    public static IRuleBuilderOptions<TElement, string> StrongPassword<TElement>(this IRuleBuilder<TElement, string> ruleBuilder, string errorMessage, int maximumLength) where TElement : class
    {
        return ruleBuilder
            .NotEmpty().WithMessage(errorMessage)
            .NotNull().WithMessage(errorMessage)
            .Length(8, maximumLength).WithMessage(errorMessage)
